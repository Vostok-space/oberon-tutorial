...
(function(vb) {
  ...
  function createAceEditor(ace, div, index, options) {
    var editor, s, sopt, eopt;
    editor = {
      div   : div,
      ace   : ace.edit(div),
      index : index
    ...
    s.setOptions(options.session || {
      tabSize               : 2,
      useSoftTabs           : true,
      navigateWithinSoftTabs: true
    });
    editor.ace.setOptions(options.editor || {
  ...
  function addEditor(box, text) {
    ...
    box.editors[ind] = createAceEditor(box.ace, d, ind, box.aceOptions);
  ...
  vb.createByDefaultIdentifiers = function(doc, ace, runners) {
    ...
    if (editors.length > 0) {
      ...
      if (len > 0 || runners.empty) {
        ...
        for (i = 0; i < editors.length; i += 1) {
          ...
          box.editors[i] = createAceEditor(ace, editors[i], i, box.aceOptions);
          if (box.tabs) {
    ...
    if (box.editors.length == 0) {
      ...
      box.editorSizeListener.observe(box.editors[0].div);
    }
